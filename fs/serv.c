...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    if ((numRead = file_read(o->o_file, ret->ret_buf, req->req_n, o->o_fd->fd_offset)) > 0)
        o->o_fd->fd_offset += numRead;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	// panic("serve_write not implemented");
     if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
    if ((numWrite= file_write(o->o_file, req->req_buf, req->req_n, o->o_fd->fd_offset)) > 0)
        o->o_fd->fd_offset += numWrite;
    return numWrite;
}
