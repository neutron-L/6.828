...
static int send_content_type(struct http_request* req)
{
    char        buf[128];
    int         r;
    const char* type;
...
void umain(int argc, char** argv)
{
    ...
    while (1) {
        ...
        if ((clientsock = accept(
                 serversock, (struct sockaddr*)&client, &clientlen)) < 0) {
