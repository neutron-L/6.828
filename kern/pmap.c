...
void page_free(struct PageInfo *pp)
{
    ...
    if (pp->pp_ref)
        panic("page_free: try to free a page which is referenced by others\n");
    if (pp->pp_link)
        panic("page_free: try to free a page which is already in free_list\n");
...
int page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
    ...
    if (!pte)
        return -E_NO_MEM;
    physaddr_t new_pa = page2pa(pp);
    physaddr_t old_pa = PTE_ADDR(*pte);

    if (old_pa != new_pa || ((*pte & 0x3FF) != (perm | PTE_P)))
    {
        if (old_pa != new_pa)
        {
