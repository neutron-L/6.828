...
struct pci_driver pci_attach_vendor[] = {
    {E1000_VENDOR_ID_82540EM, E1000_DEVICE_ID_82540EM, pci_e1000_attach},
    {0, 0, 0},
};
...
void pci_func_enable(struct pci_func* f)
{
    ...
    for (bar = PCI_MAPREG_START; bar < PCI_MAPREG_END; bar += bar_width) {
        ...
        if (size && !base)
            cprintf(
                ...
                f->bus->busno, f->dev, f->func, PCI_VENDOR(f->dev_id),
                PCI_PRODUCT(f->dev_id), regnum, base, size);
        else
    ...
    cprintf(
        ...
        f->dev, f->func, PCI_VENDOR(f->dev_id), PCI_PRODUCT(f->dev_id));
}
